...
This repository contains the official Freelens extension for CloudNativePG
(cloudnative-pg.io), the Kubernetes operator for PostgreSQL. It shows the
...
- TypeScript types for the CRDs are written in this repository from the CRD
  schemas of the pinned operator version, never imported or copied.
- "CloudNativePG" is a trademark of the Linux Foundation. The product name is
  "Freelens extension for CloudNativePG"; never present the extension as a
  CloudNativePG product.

## Safety rules (binding)

- Read-only first. Every write action (switchover, restart, reload, fencing,
  hibernation, on-demand backup, maintenance window, delete) needs its own
  spec and an explicit confirmation dialog that names the CloudNativePG
  cluster and the Kubernetes context.
- Never run user-supplied SQL from the extension's programmatic path. Queries
  executed by the extension are fixed strings.
- Do not store or forward database credentials. Prefer the metrics endpoint
  through the API server pod proxy, then `exec` with peer authentication on
  the instance socket.
- The psql terminal connects as the `postgres` superuser: say so in the UI
  (tooltip) and in the README.
- Deprecated CloudNativePG forms (for example the in-tree `barmanObjectStore`)
  are never shown or generated as the default.
...
src/
  ...
  renderer/api/<group>/     # K8s object model classes (one file per CRD and API version)
  renderer/api/types.ts     # Shared CRD typing helpers
  renderer/details/         # Detail view components for CRDs
  renderer/pages/           # Cluster page components (list views)
  renderer/menus/           # Context menu items
  renderer/preferences/     # Extension preferences UI
  renderer/components/      # Shared components (error page, available-version helper)
  renderer/icons/           # SVG icons
  common/store/             # Persisted preferences store (MobX)
...
- Detail views and pages are wrapped with `observer` from `mobx-react` and render through `withErrorPage` (see `src/renderer/components/error-page.tsx`).
