...
// The polling loop the drawers share (SPEC-0012, SPEC-0013, SPEC-0015): one
// Prometheus endpoint of a few pods, read at a fixed pace while the drawer is
// open and the window visible. One round at a time; nothing when nobody is
// looking. What is read, and how often, is the caller's choice.

import { makeObservable, observable, runInAction } from "mobx";
import { parsePrometheusText } from "../../api/instance/prometheus-text";
import { documentVisibility } from "./live-poller";

import type { PodProxyClient, ProxyFailure, ProxyResult, ProxyScheme } from "../../api/instance/pod-proxy";
import type { MetricSample } from "../../api/instance/prometheus-text";
import type { Visibility } from "./live-poller";

export interface SamplesPollTarget {
  namespace: string;
  pod: string;
  scheme: ProxyScheme;
}

export interface SamplesPollerOptions {
  client: PodProxyClient;
  /** Read at every round, so the loop follows the pods as they come and go. */
  targets: () => SamplesPollTarget[];
  /** Which endpoint of the pod to ask. */
  read: (client: PodProxyClient, target: SamplesPollTarget) => Promise<ProxyResult<string>>;
  intervalMs: number;
  visibility?: Visibility;
  now?: () => number;
}

export class SamplesPoller {
  ...
  private readonly targets: () => SamplesPollTarget[];
  private readonly read: SamplesPollerOptions["read"];
  ...
  constructor(options: SamplesPollerOptions) {
    ...
    try {
      ...
      const results = await Promise.all(
        targets.map(async (target) => ({
          target,
          result: await this.read(this.client, target),
        })),
      );
