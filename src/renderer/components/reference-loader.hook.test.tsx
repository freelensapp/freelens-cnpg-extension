...
describe("useReferenceStores", () => {
  it("loads with the namespaces the drawer knows, merging and capturing failures", async () => {
    ...
    expect(logged()).toEqual([
      "[cnpg-extension] reference store /apis/kubeswift.io/v1alpha1/swiftguests: attempt 1 ns=ns isLoaded=true items=1 lookup=hit",
    ]);
  });
  ...
  it("reports a store it could not resolve and retries it", async () => {
    ...
    expect(logged()).toHaveLength(3);
    expect(logged()[0]).toBe("[cnpg-extension] reference store swiftguests: attempt 1 ns=* store=unavailable");
  });
